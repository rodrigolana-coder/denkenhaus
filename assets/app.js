...
/* ══════════════════════════════════════════
   TRILHA
   — Todos os níveis acessíveis livremente.
   — Aulas 1-5 gratuitas, 6+ exigem plano.
══════════════════════════════════════════ */
function renderPath(li) {
  ...
  for (let n = 1; n <= total; n++) {
    ...
    else if (canOpen)                     { state='next';    cls='n-locked';  icon='🔒'; }
    else                                  { state='locked';  cls='n-locked';  icon='💎'; }

    const rowClass = n===1?'node-row':(n%3===0?'node-row right':(n%3===1?'node-row left':'node-row'));

    html += `<div class="${rowClass}">
      <div class="l-node ${cls}" onclick="openLP(${li},${n})" style="background:${state!=='locked'||canOpen?lv.color:''}" title="Aula ${n}">
...
/* ══════════════════════════════════════════
   CONCLUIR AULA — simples, sem bloqueio entre níveis
══════════════════════════════════════════ */
