...
/* ── WHATSAPP ── */
const WHATSAPP_NUMBER = '5541988601417';
...
const AULAS = {
  ...
  'Francês-A1-5': {
    icon: '📖', titulo: 'Pourquoi Paris est appelée la Ville Lumière',
  ...
  'Francês-A1-9': {
    icon: '📞', titulo: 'Coordonnées et numéros',
  ...
  'Francês-A1-17': {
    icon: '🗺️', titulo: 'Indiquer un itinéraire — Jeu',
    descricao: 'Diálogos para dar direções + jogo de labirinto interativo com 5 níveis de dificuldade!',
    duracao: '30 min', skills: ['Conversação','Vocabulário','Jogo'],
...
async function onLogin(user) {
  currentUser = user;
  // FIX: usa limit(1) em vez de single() para não quebrar quando não há resultado
...
══════════════════════════════════════════ */
const MP_LINKS = {
  basico:     'https://mpago.la/SEU-LINK-BASICO',
  professor2: 'https://mpago.la/SEU-LINK-PROF2',
  professor5: 'https://mpago.la/SEU-LINK-PROF5',
...
function renderPath(li) {
  ...
  for (let n = 1; n <= total; n++) {
    ...
    html += `<div class="${rowClass}">
      <div class="l-node ${cls}" onclick="openLP(${li},${n})" style="background:${state!=='locked'?lv.color:''}" title="Aula ${n}">
...
function openLP(li, n) {
  if (n > 5 && !hasAccess()) { showPage('plansPage'); return; }
  const lv       = LVLS[li];
  ...
  const desc     = aulaData ? aulaData.descricao : `Nesta aula você vai praticar vocabulário, gramática e compreensão.`;
  const titulo   = aulaData ? aulaData.titulo : `Aula ${n}`;
  ...
  document.getElementById('lpTitle').textContent     = `Aula ${n} — ${titulo}`;
...
async function doStart() {
  ...
  document.getElementById('lessonTopTitle').textContent       = `Aula ${n} — ${titulo}`;
...
function frGoSection(i) {
  ...
  // Reset índices ao trocar de seção
  frDlgIdx=0; frExIdx=0; frCvIdx=0;
}

function frNavDlg(dir) {
  const slides = document.querySelectorAll('.fr-dlg-slide');
  const total  = slides.length;
  if (!total) return;
  slides[frDlgIdx].style.display='none';
  frDlgIdx=Math.max(0,Math.min(total-1,frDlgIdx+dir));
  slides[frDlgIdx].style.display='block';
  const prev=document.getElementById('fr-dlg-prev');
  const info=document.getElementById('fr-dlg-info');
  const next=document.getElementById('fr-dlg-next');
  if(prev) prev.disabled=frDlgIdx===0;
  if(info) info.textContent='Diálogo '+(frDlgIdx+1)+' de '+total;
  if(next){
    if(frDlgIdx===total-1){next.textContent='Vocabulário →';next.onclick=()=>frGoSection(1);}
    else{next.textContent='Próximo →';next.onclick=()=>frNavDlg(1);}
  }
}

// FIX: frNavEx agora detecta automaticamente o total de slides
function frNavEx(dir) {
  const slides = document.querySelectorAll('.fr-ex-slide');
  const total  = slides.length;
  if (!total) return;
  slides[frExIdx].style.display='none';
  frExIdx=Math.max(0,Math.min(total-1,frExIdx+dir));
  slides[frExIdx].style.display='block';
  const prev=document.getElementById('fr-ex-prev');
  const info=document.getElementById('fr-ex-info');
  const next=document.getElementById('fr-ex-next');
  if(prev) prev.disabled=frExIdx===0;
  if(info) info.textContent='Questão '+(frExIdx+1)+' de '+total;
  if(next){
    if(frExIdx===total-1){next.textContent='Concluído ✓';next.disabled=true;}
    else{next.textContent='Próximo →';next.disabled=false;next.onclick=()=>frNavEx(1);}
  }
}

function frNavCv(dir) {
  const slides = document.querySelectorAll('.fr-cv-slide');
  const total  = slides.length;
  if (!total) return;
  slides[frCvIdx].style.display='none';
  frCvIdx=Math.max(0,Math.min(total-1,frCvIdx+dir));
  slides[frCvIdx].style.display='block';
  const prev=document.getElementById('fr-cv-prev');
  const info=document.getElementById('fr-cv-info');
  const next=document.getElementById('fr-cv-next');
  if(prev) prev.disabled=frCvIdx===0;
  if(info) info.textContent='Pergunta '+(frCvIdx+1)+' de '+total;
  if(next){next.disabled=frCvIdx===total-1;next.textContent=frCvIdx===total-1?'Concluído':'Próximo →';}
}
